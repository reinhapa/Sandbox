...
public abstract class ExceptionUtils {
  ...
  public static void changeAndThrowException(Throwable throwable, String customMessage) {
    ...
    // Attention: helper which allows to use a trick to throw a cached checked exception without a
    // wrapping exception
  ...
  private static Throwable createNewException(Throwable throwable, String message) {
    ...
    try {
      Constructor<? extends Throwable> constructor =
          throwableClass.getDeclaredConstructor(String.class);
  ...
  private static class ExceptionHelper<T extends Throwable> {
    private void throwException(Throwable exception) throws T {
      try {
        ...
        // doesn't happen with existing JVMs! - if that changes the local ClassCastException needs
        // to be ignored
