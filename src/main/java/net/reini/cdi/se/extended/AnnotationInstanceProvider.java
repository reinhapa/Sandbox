...
/**
 * <p>
 * A small helper class to create an Annotation instance of the given annotation class via
 * {@link java.lang.reflect.Proxy}. The annotation literal gets filled with the default values.
 * </p>
 * <p/>
 * <p>
 * This class can be used to dynamically create Annotations which can be usd in AnnotatedTyp. This
 * is e.g. the case if you configure an annotation via properties or XML file. In those cases you
 * cannot use {@link javax.enterprise.util.AnnotationLiteral} because the type is not known at
 * compile time.
 * </p>
 * <p>
 * usage:
...
public class AnnotationInstanceProvider implements Annotation, InvocationHandler, Serializable {
  ...
  /**
   * Required to use the result of the factory instead of a default implementation of
   * {@link javax.enterprise.util.AnnotationLiteral}.
   *
    ...
    if (!(o instanceof AnnotationInstanceProvider)) {
      if (annotationClass.isInstance(o)) {
        for (Map.Entry<String, ?> entry : memberValues.entrySet()) {
          try {
            Object oValue = annotationClass.getMethod(entry.getKey(), EMPTY_CLASS_ARRAY).invoke(o,
                EMPTY_OBJECT_ARRAY);
   ...
   * {@link Annotation#hashCode()} API docs.
   * </p>
   *
