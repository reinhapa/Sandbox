...
/**
 * Builder to aid in creation of a new {@link AnnotatedType} for use in CDI life cycle events. Using
 * the builder is typically done by reading the annotations from a {@link Class} or an
 * {@link AnnotatedType}. Once the starting class or type has been added all of annotations can be
 * modified: constructor, parameter, class, method and fields.
 * <p/>
...
public class AnnotatedTypeBuilder<X> {
  ...
  /**
   * Add an annotation to the specified field. If the field is not already present, it will be
   * added.
   *
   ...
   * @param annotationType the annotation type to remove
   * @throws IllegalArgumentException if the annotationType is null or if the field is not currently
  ...
  public AnnotatedTypeBuilder<X> removeFromField(Field field,
    ...
    if (fields.get(field) == null) {
      throw new IllegalArgumentException(
          "field " + field + " not present on class " + getJavaClass());
    } else {
  ...
  /**
   * Add an annotation to the specified method. If the method is not already present, it will be
   * added.
   *
   ...
   * @param annotationType the annotation type to remove
   * @throws IllegalArgumentException if the annotationType is null or if the method is not
   *         currently declared on the type
   */
  public AnnotatedTypeBuilder<X> removeFromMethod(Method method,
      Class<? extends Annotation> annotationType) {
    if (methods.get(method) == null) {
      throw new IllegalArgumentException(
          "Method " + method + " not present on class" + getJavaClass());
    } else {
  ...
  /**
   * Add an annotation to the specified method parameter. If the method is not already present, it
   * will be added. If the method parameter is not already present, it will be added.
   *
   * @param method the method to add the annotation to
   ...
   * @param annotationType the annotation type to remove
   * @throws IllegalArgumentException if the annotationType is null, if the method is not currently
   *         declared on the type or if the parameter is not declared on the method
   */
  public AnnotatedTypeBuilder<X> removeFromMethodParameter(Method method, int position,
    ...
    if (methods.get(method) == null) {
      ...
      if (methodParameters.get(method).get(position) == null) {
        throw new IllegalArgumentException(
            String.format("parameter %s not present on method %s declared on class %s", method,
                position, getJavaClass()));
      } else {
  ...
  /**
   * Add an annotation to the specified constructor. If the constructor is not already present, it
   * will be added.
   *
   ...
   * @throws IllegalArgumentException if the annotationType is null or if the constructor is not
   *         currently declared on the type
   */
   ...
   * @param constructor the constructor to add the annotation to
   * @param annotationType the annotation to add
   * @throws IllegalArgumentException if the annotationType is null, if the annotation does not
   *         exist on the type or if the constructor is not currently declared on the type
   */
  ...
  /**
   * Add an annotation to the specified constructor parameter. If the constructor is not already
   * present, it will be added. If the constructor parameter is not already present, it will be
   * added.
   *
   * @param constructor the constructor to add the annotation to
   * @param position the position of the parameter to add
   ...
   * @param position the position of the parameter to remove
   * @param annotationType the annotation type to remove
   * @throws IllegalArgumentException if the annotationType is null, if the constructor is not
   *         currently declared on the type or if the parameter is not declared on the constructor
   */
   ...
   * @param annotationType the annotation type to remove
   * @throws IllegalArgumentException if the annotationType is null, if the callable which declares
   *         the parameter is not currently declared on the type or if the parameter is not declared
   *         on either a constructor or a method
   */
  public AnnotatedTypeBuilder<X> removeFromParameter(AnnotatedParameter<? super X> parameter,
    ...
    if (parameter.getDeclaringCallable().getJavaMember() instanceof Constructor<?>) {
      ...
      Constructor<X> constructor =
          (Constructor<X>) parameter.getDeclaringCallable().getJavaMember();
  ...
  /**
   * Add an annotation to the specified parameter. If the callable which declares the parameter is
   * not already present, it will be added. If the parameter is not already present on the callable,
   * it will be added.
   *
   * @param parameter the parameter to add the annotation to
   * @param annotation the annotation to add
   * @throws IllegalArgumentException if the annotation is null or if the parameter is not declared
   *         on either a constructor or a method
   */
  ...
  /**
   * Remove annotations from the type, and all of it's members. If an annotation of the specified
   * type appears on the type declaration, or any of it's members it will be removed.
   *
  ...
  public AnnotatedTypeBuilder<X> removeFromAll(Class<? extends Annotation> annotationType) {
    if (annotationType == null) {
      throw new IllegalArgumentException(
          String.format("%s parameter must not be null", "annotationType"));
    }
    ...
    for (Map.Entry<Constructor<?>, AnnotationBuilder> constructor : constructors.entrySet()) {
      ...
      if (constructorParameters.get(constructor.getKey()) != null) {
        for (Map.Entry<Integer, AnnotationBuilder> parameter : constructorParameters
            .get(constructor.getKey()).entrySet()) {
          parameter.getValue().remove(annotationType);
        }
      }
    }
  ...
  /**
   * Reads in from an existing AnnotatedType. Any elements not present are added. The javaClass will
   * be read in. If the annotation already exists on that element in the builder the read annotation
   * will be used.
   *
  ...
  /**
   * Reads in from an existing AnnotatedType. Any elements not present are added. The javaClass will
   * be read in if overwrite is true. If the annotation already exists on that element in the
   * builder, overwrite determines whether the original or read annotation will be used.
   *
   * @param type the type to read from
   * @param overwrite if true, the read annotation will replace any existing annotation
   * @throws IllegalArgumentException if type is null
   */
  public AnnotatedTypeBuilder<X> readFromType(AnnotatedType<X> type, boolean overwrite) {
    ...
    for (AnnotatedMethod<? super X> method : type.getMethods()) {
      ...
      for (AnnotatedParameter<? super X> p : method.getParameters()) {
        ...
        if (methodParameters.get(method.getJavaMember()).get(p.getPosition()) == null) {
          methodParameters.get(method.getJavaMember()).put(p.getPosition(),
    ...
    for (AnnotatedConstructor<? super X> constructor : type.getConstructors()) {
      ...
      mergeAnnotationsOnElement(constructor, overwrite,
      ...
      for (AnnotatedParameter<? super X> p : constructor.getParameters()) {
        if (constructorParameters.get(constructor.getJavaMember()) == null) {
          constructorParameters.put(constructor.getJavaMember(),
              new HashMap<Integer, AnnotationBuilder>());
        }
  ...
  /**
   * Reads the annotations from an existing java type. Annotations already present will be
   * overwritten
   *
   * @param type the type to read from
   * @throws IllegalArgumentException if type is null
  ...
  /**
   * Reads the annotations from an existing java type. If overwrite is true then existing
    ...
    for (Method method : ReflectionUtils.getAllDeclaredMethods(type)) {
      ...
      for (int i = 0; i < method.getParameterTypes().length; ++i) {
        ...
        for (Annotation annotation : method.getParameterAnnotations()[i]) {
          if (overwrite
              || !parameterAnnotationBuilder.isAnnotationPresent(annotation.annotationType())) {
  ...
  /**
   * Create an {@link AnnotatedType}. Any public members present on the underlying class and not
   * overridden by the builder will be automatically added.
   */
  public AnnotatedType<X> create() {
    ...
    for (Map.Entry<Method, Map<Integer, AnnotationBuilder>> parameters : methodParameters
        .entrySet()) {
    ...
    for (Map.Entry<Constructor<?>, Map<Integer, AnnotationBuilder>> parameters : constructorParameters
        .entrySet()) {
      Map<Integer, AnnotationStore> parameterAnnotations = new HashMap<Integer, AnnotationStore>();
    ...
    return new AnnotatedTypeImpl<X>(javaClass, typeAnnotations.create(), fieldAnnotations,
        methodAnnotations, methodParameterAnnotations, constructorAnnotations,
        constructorParameterAnnotations, fieldTypes, methodParameterTypes,
   ...
   */
  public AnnotatedTypeBuilder<X> overrideMethodParameterType(Method method, int position,
      Type type) {
    if (method == null) {
  ...
  public AnnotatedTypeBuilder<X> overrideConstructorParameterType(Constructor<X> constructor,
    ...
    if (constructor == null) {
      throw new IllegalArgumentException(
          String.format("%s parameter must not be null", "constructor"));
    }
    if (type == null) {
      throw new IllegalArgumentException(String.format("%s parameter must not be null", "type"));
    }
